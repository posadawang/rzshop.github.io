import { logout, requireAuth } from "./firebase.js";

const CartStorageKey = "cart_items";
const DEFAULT_THUMBNAIL = "assets/images/logo.svg";
...
if (typeof window !== "undefined") {
  ...
  window.addEventListener("storage", event => {
    ...
    const items = typeof event.newValue === "string" ? parseCartItems(event.newValue) : [];
...
// 載入購物車
function renderCart(items = Cart.get()) {
  const cart = Array.isArray(items) ? items : Cart.get();
...
async function initCatalogPage() {
  ...
  cardsContainer.innerHTML = `
    <div class="col-12">
      <div class="text-center text-muted py-5">商品載入中...</div>
    </div>`;
      ...
      cardsContainer.innerHTML = `
        <div class="col-12">
          <div class="text-center text-muted py-5 border rounded-3">
            ${allItems.length ? "找不到符合條件的商品，請嘗試其他關鍵字。" : "目前暫時沒有商品可以展示，請稍後再試。"}
          </div>
        </div>`;
      return;
    }
      ...
      .map(item => `
        <div class="col-12 col-sm-6 col-lg-4">
          <div class="card h-100 shadow-sm">
            <img src="${item.thumbnail || DEFAULT_THUMBNAIL}" class="card-img-top" alt="${item.title}">
      ...
      <div class="col-md-6">
        ...
        ${gallery.length > 1 ? `
          <div class="d-flex gap-2 mt-3 flex-wrap">
            ${gallery
              .map(image => `<img src="${image || DEFAULT_THUMBNAIL}" alt="${product.title}" class="img-thumbnail" style="width:90px;height:90px;object-fit:cover;">`)
              .join("")}
          </div>
        ` : ""}
      </div>
